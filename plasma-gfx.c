// Plasma at pixel resolution — termpaint + kitty graphics protocol.
// The classic demoscene plasma: several sine fields summed together and
// mapped through a cycling cosine palette, so the whole frame breathes
// and shifts colour. Falls back to a colour-ramp cell field on terminals
...
#define FRAME_MS 50     // 20fps — kitty RGBA frames are large; tune with PLASMA_FPS
...
static float cx = 0.5f, cy = 0.5f;   // radial wave centre, normalised
...
// Summed sine field at normalised (u, v); ~[-4, 4]. `ar` corrects the
// radial term so the moving wave centre stays circular on wide screens.
...
//
// The plasma fills the frame opaquely; alpha fades to nothing over the top
...
static void redraw(void) {
    ...
    if (pixel_mode) {
        // the image supplies the plasma; cells only carry the status bar
        termpaint_surface_clear(surface, TERMPAINT_DEFAULT_COLOR,
                                TERMPAINT_DEFAULT_COLOR);
        snprintf(buf, sizeof(buf),
                 " space: reseed  click: centre  a: %s  c: %s  +/-: speed x%.2f  q: quit │ kitty gfx %dx%d ",
...
int main(void) {
    ...
    if (kitty_ok) {
        int max_dim = 512;  // ~1MB RGBA per frame; use PLASMA_MAXDIM=1024 for sharper
        const char *s = getenv("PLASMA_MAXDIM");
        if (s && atoi(s) >= 128) {
            max_dim = atoi(s);
        }
    ...
    const char *fps_env = getenv("PLASMA_FPS");
