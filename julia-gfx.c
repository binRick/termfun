// Julia set at pixel resolution — termpaint + kitty graphics protocol.
// Each pixel iterates z = z*z + c and is shaded by a smooth (continuous)
// escape count mapped through a cycling cosine palette, so the fractal
// breathes and shifts colour. The constant c drifts on a loop just inside
// the main cardioid. Falls back to a colour glyph-ramp cell field on
...
#define FRAME_MS 50     // 20fps — kitty RGBA frames are large; tune with JULIA_FPS
...
//
// The fractal fills the frame opaquely; alpha fades to nothing over the top
...
// Smooth-escape ramp: sparse glyphs near the boundary, dense deeper in.
...
static void redraw(void) {
    ...
    if (pixel_mode) {
        // the image supplies the fractal; cells only carry the status bar
        termpaint_surface_clear(surface, TERMPAINT_DEFAULT_COLOR,
                                TERMPAINT_DEFAULT_COLOR);
        snprintf(buf, sizeof(buf),
                 " space: jump  click: set c  a: %s  c: %s  +/-: iters %d  q: quit │ kitty gfx %dx%d ",
...
int main(void) {
    ...
    if (kitty_ok) {
        int max_dim = 512;  // ~1MB RGBA per frame; use JULIA_MAXDIM=1024 for sharper
        const char *s = getenv("JULIA_MAXDIM");
        if (s && atoi(s) >= 128) {
            max_dim = atoi(s);
        }
    ...
    const char *fps_env = getenv("JULIA_FPS");
